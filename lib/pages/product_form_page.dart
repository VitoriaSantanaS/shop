...
class _ProductFormPageState extends State<ProductFormPage> {
  ...
  bool isValidImageUrl(String url) {
    ...
    bool endsWithFile = url.toLowerCase().endsWith('.png') || url.toLowerCase().endsWith('.jpg') || url.toLowerCase().endsWith('.jpeg') || url.toLowerCase().endsWith('.webp');
  ...
  Future<void> _submitForm() async {
    ...
    if (_formData['name'] == null ||
        ...
        _formData['imageUrl'] == null) return;
                         ...
                         children: <Widget>[
                           TextFormField(
                             ...
                             validator: (_name) {
                               ...
                               if (name.trim().length < 3) return 'O nome deve conter pelo menos 3 letras';
                               return null;
                             },
                           ),
                           TextFormField(
                             ...
                             onSaved: (price) => _formData['price'] = double.tryParse(price ?? '0') ?? 0.0,
                             // ignore: no_leading_underscores_for_local_identifiers
                             validator: (_price) {
                               final priceString = _price ?? '';
                               final price = double.tryParse(priceString);
                              
                               if (price == null || price <= 0) return 'Informe um preço válido';
                               return null;
                              },
                            ),
                           TextFormField(
                             ...
                             validator: (description) {
                              ...
                              if (_description.trim().isEmpty) return 'Descrição é obrigatória';
                              if (_description.trim().length < 10) return 'O nome deve conter pelo menos 10 letras';
                              return null;
                              },
                            ),
                           Row(
                                 ...
                                 child: 
                                   TextFormField(
                                     ...
                                     validator: (url) {
                                      ...
                                      if (!isValidImageUrl(imageUrl)) return 'informe uma Url valida';
                                      return null;
                                    },
                                  ),
                                ),
